<%@ page import="admin.food_details_back" %>
...
<div class="box1">
	...
	if(hotel_name != null && !hotel_name.trim().isEmpty()) {
	    ResultSet rs = food_details_back.foodDetails(hotel_name);
	...
	while(rs.next()){
		...
		+"<td class='content' id='ph'>"+rs.getLong(3)+"</td>"
				+ "<td class='content'> <form action='food_delete' method='post'>"+
				"<input type='hidden' name='hotel_name' value='" + hotel_name + "'>" +"<button onclick='delete_msg()' class='btn' name='delete_food' value="+ food_name+">Delete</button></form></td>" );
	}
	out.print("</table>");
	out.print("</div>");
	}
